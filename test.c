...
BTNode *RemoveBST(BTNode **Root, DATATYPE Target)
{
   ...
   else if (RetSubTreeLeft(TargetNode) == NULL || RemoveSubTreeRight(TargetNode) == NULL)
   {
      ...
      {
         ChildofTarget = RemoveSubTreeRight(TargetNode);
      }
   ...
   else
   {
      ...
      BTNode *MaxNode = RetSubTreeLeft(TargetNode);
      BTNode *ParentofMax = TargetNode;

      DATATYPE Backup;

      //find node to replace target node
      while (RetSubTreeRight(MaxNode) != NULL)
      {
         ParentofMax = MaxNode;
         MaxNode = RetSubTreeRight(MaxNode);
      }
      ...
      //link parent and child of MaxNode
      if (RetSubTreeRight(ParentofMax) == MaxNode)
      {
         ChangeSubTreeRight(ParentofMax, RetSubTreeLeft(MaxNode));
      }
      else
      {
         ChangeSubTreeLeft(ParentofMax, RetSubTreeLeft(MaxNode));
      }
